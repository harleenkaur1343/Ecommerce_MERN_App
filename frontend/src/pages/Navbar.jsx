  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur-md border-b border-primary/5 shadow-md shadow-primary/5">
      <div className="container mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
        <Link to="/" className="font-display font-bold text-2xl tracking-tighter text-foreground flex items-center gap-2 group cursor-pointer">
          ...
          {navLinks.map((link) => (
            <Link 
              ...
              className={cn(
                ...
                location.pathname === link.href ? "text-primary" : "text-muted-foreground/80"
              )}
            >
              {link.name}
            </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="p-2.5 bg-primary text-primary-foreground rounded-full hover:opacity-90 transition-all relative cursor-pointer shadow-lg shadow-primary/20">
            ...
            <span className="absolute -top-1 -right-1 w-4 h-4 bg-foreground text-background text-[9px] flex items-center justify-center rounded-full font-black">2</span>
          </button>
